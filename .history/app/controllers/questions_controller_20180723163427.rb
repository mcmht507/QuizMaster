class QuestionsController < ApplicationController
  ...
  # GET /questions
  def index
    query = "SELECT questions.question_id question_id,questions.content content,questions.answer answer,questions.category_id category_id,questions.user_id user_id,questions.created_at created_at,questions.updated_at updated_at,answers.answer_id answer_id,answers.is_correct is_correct,categories.content category_content FROM questions LEFT OUTER JOIN categories ON categories.category_id = questions.category_id LEFT OUTER JOIN (select answer_id,question_id,is_correct from answers where user_id = :user_id) answers ON answers.question_id = questions.question_id"
  ...
  def create
    ...
    if question.save
      res_success({question_id:question.question_id,content:question.content})
    else
  ...
  private
    ...
    def set_question
      ...
      query = "SELECT questions.question_id question_id,questions.content content,questions.answer answer,categories.category_id category_id,questions.user_id user_id, questions.created_at created_at,questions.updated_at updated_at,answers.answer_id answer_id,answers.is_correct is_correct,categories.content category_content FROM questions LEFT OUTER JOIN categories ON categories.category_id = questions.category_id LEFT OUTER JOIN (select answer_id,question_id,is_correct from answers where user_id = :user_id) answers ON answers.question_id = questions.question_id where questions.question_id = :question_id"
