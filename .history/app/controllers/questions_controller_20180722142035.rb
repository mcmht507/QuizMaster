class QuestionsController < ApplicationController
  ...
  def index
    ...
    query = "SELECT questions.question_id question_id,questions.content content,questions.user_id user_id,questions.created_at created_at,questions.updated_at updated_at,answers.answer_id answer_id,answers.Correct Correct,categories.* FROM questions LEFT OUTER JOIN categories ON categories.category_id = questions.category_id LEFT OUTER JOIN (select answer_id,question_id,is_correct from answers where user_id = :user_id) answers ON answers.question_id = questions.question_id"
