class QuestionsController < ApplicationController
  ...
  # GET /questions
  def index
    user_answer_temp = Answer.arel_table
    question_table = Question.arel_table

    user_answer_table = user_answer_temp
                          .project(Arel.sql('*'))
                          .where(user_id: @login_user.user_id)
                          .as('user_answer_table') # これ

    join_conds = question_table
                 .join(user_answer_table, Arel::Nodes::InnerJoin)
                 .on(user_answer_table[:question_id]
                 .eq(question_table[:question_id]))
                 .join_sources
