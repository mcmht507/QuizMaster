class QuestionsController < ApplicationController
  ...
  private
    ...
    def set_question
      ...
      query = "SELECT questions.question_id question_id,questions.content content, questions.answer answer, content,questions.user_id user_id,questions.created_at created_at,questions.updated_at updated_at,answers.answer_id answer_id,answers.is_correct is_correct,categories.category_id category_id,categories.content category_content FROM questions LEFT OUTER JOIN categories ON categories.category_id = questions.category_id LEFT OUTER JOIN (select answer_id,question_id,is_correct from answers where user_id = :user_id) answers ON answers.question_id = questions.question_id where questions.question_id = :question_id"
