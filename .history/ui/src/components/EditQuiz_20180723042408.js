...
export default class EditQuiz extends React.Component {
  ...
  render() {
    ...
    console.log("this.state.is_init:" + this.state.is_init);
    let input_category_id = this.state.category_id
    let inputQuestion = this.state.question_content
    let inputAnswer = this.getAnswer();
    if (question) {
      this.setState({
        category_id : !input_category_id ? question.category_id : input_category_id,
        question_content : !inputQuestion ? question.content : inputQuestion,
    ...
    return (
      <div key={"editQuiz"}>
        ...
        <form onSubmit={handleSubmit(this.sendItems)} style={{ width: '80%', margin: '0 auto', textAlign: 'center' }}>
          <div>
            <CategoryList history={this.props.history} category_id={category_id} updateCategoryId={this.updateCategoryId.bind(this)} />
