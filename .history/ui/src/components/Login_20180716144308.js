...
@reduxForm({
  ...
  validate: values => {
    ...
    const invalidErrMsg = '{0} is is a required filed'
    const requiredErrMsg = '{0} is is a required filed'
    // 入力変更時にパラメータが渡ってくる
    const errors = {}
    if (!values.userid) {
      errors.userid = requiredErrMsg.replace("{0}","Userid")
    }
    if (!values.name) {
      errors.name = requiredErrMsg.replace("{0}", "Name")
    }
    if (!values.email) {
      errors.email = requiredErrMsg.replace("{0}", "")
    } else if (!/^[A-Z0-9._%+-]+@[A-Z0-9.-]+\.[A-Z]{2,4}$/i.test(values.email)) {
      errors.email = invalidErrMsg.replace("{0}", "")
    }
    if (!values.password) {
