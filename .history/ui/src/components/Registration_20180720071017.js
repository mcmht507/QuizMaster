...
@reduxForm({
  ...
  validate: values => {
    ...
    const errors = {}
    if (!values.user_id) {
      errors.user_id = commonConst.requiredErrMsg.replace("{0}", "Userid")
    }
    if (!values.name) {
      errors.name = commonConst.requiredErrMsg.replace("{0}", "Name")
    }
    if (!values.email) {
      errors.email = commonConst.requiredErrMsg.replace("{0}", "Email")
    } else if (!/^[A-Z0-9._%+-]+@[A-Z0-9.-]+\.[A-Z]{2,4}$/i.test(values.email)) {
      errors.email = commonConst.invalidErrMsg.replace("{0}", "Email")
    }
    if (!values.password) {
